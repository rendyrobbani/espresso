...
public final class SubfungsiClassificationFactory {
	...
	@Getter
	private static final class SubfungsiClassificationImpl implements SubfungsiClassification {
		...
		private SubfungsiClassificationImpl(String fungsiPart, String subfungsiPart) {
			this.fungsiPart = fungsiPart;
			this.subfungsiPart = subfungsiPart;

			this.fungsiCode = this.fungsiPart;
			this.subfungsiCode = this.fungsiCode + "." + this.subfungsiPart;
		}
	...
	public static SubfungsiClassification classify(String code) {
		...
		var matcher = Pattern.compile(SubfungsiClassification.SUBFUNGSI_REGEX_GROUP).matcher(code);
		if (matcher.matches()) return new SubfungsiClassificationImpl(matcher.group(1), matcher.group(2));
